...
namespace WineBridgePlugin.Processes
{
    public static class LinuxProcessMonitor
    {
            ...
            CancellationTokenSource watcherToken)
        {
            ...
            try
            {
                ...
                if (!ready)
                {
                    ...
                    await InvokeStopEvent(instance, null);
                    return;
                }

                stopwatch = await InvokeStartEvent(instance, process);
                ...
                await InvokeStopEvent(instance, stopwatch);
            }
            catch (Exception e)
            {
        ...
        private static async Task<Stopwatch> InvokeStartEvent(
            PlayController instance,
            LinuxProcess process)
        {
            try
