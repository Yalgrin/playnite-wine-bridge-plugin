...
How does it work? It uses [Harmony](https://harmony.pardeike.net/) to patch specific methods in Playnite and the Steam
...
commands from Playnite, which in turn allows me to launch any game regardless of how it was installed. However, it quickly became cumbersome to write custom play actions for every game I wanted to launch. And thus, this plugin
was created.
...
1. Install [Playnite](https://playnite.link/) inside a [Wine Prefix](https://www.winehq.org/) on your Linux machine.
   ...
   the prefix using `winetricks` and once the app is installed enable the following option: Settings > Advanced >
...
3. Make sure the Linux script has execution permissions. The plugin should do that by itself upon first startup. If not, run the following command in the terminal (replace paths as needed):
...
_A lot of these paths are using the default Wine drive mappings, meaning the root folder `/` is mapped to the `Z:` drive. If you decide to change those you have to adjust the following parameters accordingly._
...
- If you want to use Steam integration, check the **Enable Steam integration** option and configure the following parameters:
  ...
  - _If you have installed the Steam client natively on your Linux machine, all you likely need to do is change the `user` folder to your actual username. If you use Steam client installed as a Flatpak, the parameters should look more or less like this:_
...
```
Make sure the search term finds only the game and not any other processes. It's worth mentioning that `pgrep` is case sensitive, so pay attention when choosing the proper search term. Once you're done, write down the final expression.
